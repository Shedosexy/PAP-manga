...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .admin-header { padding: 24px 32px 0; }
    .admin-header-inner {
    ...
    .stat-card::before {
      ...
      height: 3px; background: var(--sc-color, var(--card-border));
    ...
    .stat-icon { font-family: var(--font-mono); font-size: 0.58rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--sc-color, var(--grey)); font-weight: 700; }
    .stat-value { font-family: var(--font-display); font-size: clamp(1.1rem, 1.4vw, 1.5rem); font-weight: 900; line-height: 1; color: var(--black); }
    .stat-label { font-family: var(--font-mono); font-size: 0.55rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--grey); }
    .stat-link { display: inline-flex; align-items: center; gap: 5px; margin-top: 4px; font-family: var(--font-mono); font-size: 0.52rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--sc-color, var(--accent)); text-decoration: none; opacity: 0.75; transition: opacity 0.2s; }
    ...
    .panel-card {
      background: rgba(255,255,255,0.96); border: 1.5px solid var(--card-border);
      border-radius: 18px; overflow: hidden; box-shadow: 0 14px 36px rgba(0,0,0,0.04);
    }
    .panel-card-header {
      padding: 16px 18px 14px; border-bottom: 1.5px solid var(--light-grey);
      display: flex; align-items: center; justify-content: space-between; gap: 12px;
    }
    .panel-card-title { font-family: var(--font-mono); font-size: 0.68rem; letter-spacing: 0.18em; text-transform: uppercase; color: var(--black); font-weight: 700; }
    .panel-card-more { font-family: var(--font-mono); font-size: 0.58rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--accent); text-decoration: none; transition: opacity 0.2s; opacity: 0.7; }
    .panel-card-more:hover { opacity: 1; }
    .panel-table-wrap { overflow-x: auto; }
    .panel-table { width: 100%; border-collapse: collapse; }
    .panel-table--wide { min-width: 760px; }
    .panel-table th { font-family: var(--font-mono); font-size: 0.58rem; letter-spacing: 0.15em; text-transform: uppercase; color: var(--grey); padding: 12px 18px 10px; text-align: left; border-bottom: 1px solid var(--light-grey); }
    ...
    .estado-badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-family: var(--font-mono); font-size: 0.58rem; letter-spacing: 0.1em; text-transform: uppercase; }
    ...
    .contacto-card { background: rgba(255,255,255,0.96); border: 1.5px solid var(--card-border); border-radius: 16px; padding: 18px 18px 16px; }
    ...
    .livro-badge { background: var(--off-white); border: 1px solid var(--card-border); border-radius: 6px; padding: 3px 10px; font-size: 0.72rem; font-family: var(--font-body); color: var(--black); }
    .btn-toggle-livros { background: var(--black); color: var(--white); border: none; border-radius: 4px; padding: 5px 14px; font-family: var(--font-mono); font-size: 0.6rem; letter-spacing: 0.1em; text-transform: uppercase; cursor: pointer; transition: background 0.2s; }
    ...
    @media (max-width: 1100px) {
      ...
      .stats-section, .chart-section, .tables-section, .contactos-section, .compras-section { width: calc(100% - 40px); }
    ...
    @media (max-width: 768px) {
      ...
      .chart-card { width: 100%; max-width: 360px; }
      .panel-card-header { align-items: flex-start; flex-direction: column; }
      .panel-table--wide { min-width: 680px; }
    }
    @media (max-width: 560px) {
      .admin-header, .stats-section, .chart-section, .tables-section, .contactos-section, .compras-section { width: calc(100% - 28px); padding-left: 0; padding-right: 0; }
  ...
  <section class="chart-section">
    <div class="chart-card">
      ...
      <div class="chart-card-body">
        <canvas id="chart-categorias" style="max-width: 700px; max-height: 700px;"></canvas>
      </div>
    </div>
  </section>
      ...
      <div class="panel-card">
        ...
        <div class="panel-table-wrap">
          <table class="panel-table panel-table--wide">
            <thead>
              <tr>
                <th>#</th><th>Cliente</th><th>Total</th><th>Estado</th><th>Data</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div class="panel-card">
        ...
        <div class="panel-table-wrap">
          <table class="panel-table">
            <thead>
              <tr>
                <th>#</th><th>Utilizador</th><th>Assunto</th><th>Data</th>
              </tr>
            </thead>
            <tbody>
    ...
    <div class="panel-card">
      ...
      <div class="panel-table-wrap">
        <table class="panel-table panel-table--wide">
          <thead>
            <tr>
              <th>Cliente</th><th>Email</th><th>Encomendas</th><th>Itens</th><th>Livros</th>
            </tr>
          </thead>
          <tbody>
...
new window.Chart(document.getElementById('chart-categorias'), {
  type: 'doughnut',
  data: {
    labels: catLabels,
    datasets: [{
  ...
  options: {
    ...
    plugins: {
      legend: {
        position: 'bottom',
        labels: { color: '#999', font: { size: 11 }, padding: 10 }
      }
    }
  }
});
